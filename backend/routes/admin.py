...
async def update_book(book_id: int, book: schemas.BookCreate, db: AsyncSession = Depends(async_session)):
...
async def delete_book(book_id: int, db: AsyncSession = Depends(async_session)):
    result = await db.execute(select(models.Book).filter(models.Book.id == book_id))
    db_book = result.scalars().first()
    if not db_book:
        raise HTTPException(status_code=404, detail="Book not found")
...
async def get_orders(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(async_session)):
...
async def update_order_status(order_id: int, status: str, db: AsyncSession = Depends(async_session)):
...
async def healthcheck(db: AsyncSession = Depends(async_session)):
